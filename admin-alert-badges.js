import'./admin-activity-center.js?v=1';
...
async function load(){const role=document.body.dataset.role||'student',supportAccess=['owner','admin','support'].includes(role);const [messages,orders]=await Promise.all([supportAccess?read('messages'):[],supportAccess?read('orders'):[]]);const supportMessages=messages.filter(isPendingMessage),supportOrders=orders.filter(isPendingOrder);paint('admin-messages.html',supportMessages.length,'رسائل جديدة تحتاج متابعة');paint('admin-orders.html',supportOrders.length,'طلبات خدمات جديدة تحتاج متابعة')}
