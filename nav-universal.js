(() => {
  ...
  const adminModules = async () => {
    try {
      const { requireAdmin } = await import('./admin-access.js?v=11'); const session = await requireAdmin(); if (page === 'admin-community.html') { location.replace('admin-analytics.html'); return; }
