(() => {
  ...
  if (!isAdmin && !nav.querySelector('.desktop-student-nav')) {
    ...
    desktop.innerHTML = `${item('profile.html','profile','حسابي','class="student-profile-link"')}${item('scholarships.html','scholarship','المنح')}${item('documents.html','documents','مستنداتي')}${item('currency.html','currency','العملات')}${item('services.html','services','الخدمات')}${item('offers.html','offers','العروض')}<div class="desktop-more"><button type="button" class="desktop-more-button" aria-expanded="false"><span class="desktop-nav-icon">${icons.grid}</span><span>المزيد</span><i>⌄</i></button><div class="desktop-more-menu" aria-hidden="true">${item('index.html','home','الرئيسية')}${item('contact.html','contact','تواصل معنا')}${item('about.html','about','من نحن')}<div class="desktop-admin-slot"></div></div></div>`;
  ...
  const publicModules = async () => {
    const routes = { 'index.html': async () => { await load('./homepage-fixes.js?v=11','homepage setup'); await load('./homepage-live.js?v=12','homepage data'); }, 'scholarships.html': () => Promise.all([load('./scholarships-live.js?v=20260902full','scholarships'), load('./scholarship-favorites.js?v=10','favorites')]), 'documents.html': () => load('./documents.js?v=81','documents tools'), 'services.html': () => load('./public-commerce-live.js?v=21','services'), 'offers.html': () => load('./public-commerce-live.js?v=21','offers'), 'contact.html': () => load('./contact-live.js?v=11','contact'), 'profile.html': () => load('./scholarship-favorites.js?v=11','favorites') };
    await routes[page]?.();
    onIdle(() => { if (!['login.html','register.html'].includes(page)) load('./nav-auth.js?v=102','navigation account state'); load('./analytics.js?v=10','analytics'); if (!['login.html','register.html','terms.html','privacy.html'].includes(page)) load('./notifications-live.js?v=12','notifications'); load('./ads.js?v=1','ads'); });
  };
  const adminModules = async () => {
    try {
      ...
      document.body.dataset.role = session.role; root.classList.remove('admin-pending'); await load('./admin-navigation.js?v=19','admin navigation'); load('./admin-mobile.js?v=11','admin mobile navigation'); load('./admin-alert-badges.js?v=10','admin alerts');
      const routes = { 'admin-analytics.html':'./admin-analytics.js?v=10', 'admin-homepage.html':'./admin-live-data.js?v=10', 'admin-scholarships.html':'./scholarships-admin.js?v=10', 'admin-users.html':'./admin-users.js?v=10', 'admin-student.html':'./admin-student.js?v=10', 'admin-staff.html':'./admin-staff.js?v=10', 'admin-gamification.html':'./admin-gamification.js?v=10', 'admin-services.html':'./admin-commerce.js?v=21', 'admin-offers.html':'./admin-commerce.js?v=21', 'admin-orders.html':'./admin-orders.js?v=10', 'admin-messages.html':'./admin-messages.js?v=11', 'admin-announcements.html':'./admin-announcements.js?v=22' };
