...
if (cards) {
  ...
  function filtered(){const term=search.value.trim().toLowerCase();return scholarships.filter(item=>(country==='all'||item.country===country)&&(level.value==='all'||(item.studyLevels||[]).includes(level.value))&&(status.value==='all'||dateState(item).key===status.value)&&(!term||searchable(item).includes(term)));}
  function render(){const items=filtered();count.textContent=`${items.length} منحة`;empty.hidden=items.length>0;cards.hidden=items.length===0;cards.innerHTML=items.map(item=>{const slug=item.slug||item.id,state=dateState(item),opens=item.openDate||item.openingDate,closes=item.deadline||item.closeDate||item.endDate,levels=(item.studyLevels||[]).slice(0,3);return `<article class="scholarship-card scholarship-card-clean status-${state.tone}" data-scholarship-card><div class="scholarship-card-head"><div class="country-badge-large"><span>${flag(item.country)}</span><b>${esc(item.country||'دولي')}</b></div><span class="scholarship-state ${state.tone}"><i></i>${esc(state.label)}</span></div><div class="scholarship-provider">${esc(item.provider||'جهة مانحة')}</div><h2>${esc(item.title)}</h2><p class="scholarship-summary">${esc(item.shortDescription||'منحة دراسية — افتح الدليل لمعرفة التفاصيل والشروط.')}</p><div class="scholarship-dates"><div><small>تاريخ الفتح</small><b>${formatDate(opens)}</b></div><span>←</span><div><small>تاريخ الإغلاق</small><b>${formatDate(closes)}</b></div></div><div class="scholarship-quick-info"><span>💰 ${esc(item.funding||'التمويل حسب البرنامج')}</span><span>🎓 ${levels.map(esc).join(' • ')||'المراحل حسب البرنامج'}</span></div><div class="scholarship-card-actions"><button type="button" class="scholarship-favorite-btn" data-favorite-slug="${esc(slug)}" data-favorite-title="${esc(item.title)}" data-favorite-country="${esc(item.country||'')}" aria-label="حفظ المنحة" aria-pressed="false">♡</button><a class="btn primary scholarship-details-btn" href="scholarship.html?slug=${encodeURIComponent(slug)}">عرض التفاصيل</a></div></article>`;}).join('');document.dispatchEvent(new CustomEvent('shadrat:scholarships-rendered'));}
  ...
  document.querySelector('#reset-scholarship-filters')?.addEventListener('click',()=>{search.value='';level.value='all';status.value='all';country='all';countryButtons.forEach(b=>b.classList.toggle('is-active',b.dataset.country==='all'));render();search.focus();});
  render();
