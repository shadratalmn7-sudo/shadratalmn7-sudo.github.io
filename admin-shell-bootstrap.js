(()=>{
  ...
  function ensureCurrentCss(){return new Promise((resolve,reject)=>{const existing=document.querySelector('link[data-admin-navigation-current="32"],link[data-admin-navigation-css="32"]');if(existing){if(existing.sheet)return resolve();existing.addEventListener('load',resolve,{once:true});existing.addEventListener('error',()=>reject(new Error('admin navigation css failed')),{once:true});return}const link=document.createElement('link');link.rel='stylesheet';link.href='admin-navigation.css?v=32';link.dataset.adminNavigationCurrent='32';link.addEventListener('load',resolve,{once:true});link.addEventListener('error',()=>reject(new Error('admin navigation css failed')),{once:true});document.head.appendChild(link)})}
  async function boot(){await waitForRole();const cssReady=ensureCurrentCss();const navReady=import('./admin-navigation.js?v=32');await Promise.all([cssReady,navReady]);const aside=document.querySelector('.admin-nav');if(aside&&aside.dataset.adminBuilt!=='32')aside.dataset.adminBuilt='force-current-32';document.body.dataset.adminShellReady='32';root.classList.remove('admin-pending','admin-shell-pending')}
