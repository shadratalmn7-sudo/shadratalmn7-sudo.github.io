(()=>{
  ...
  if(!window.__shadratAdminEnglishLocale){
    ...
    AdminDateTimeFormat.prototype=NativeDateTimeFormat.prototype;AdminDateTimeFormat.supportedLocalesOf=NativeDateTimeFormat.supportedLocalesOf.bind(NativeDateTimeFormat);Intl.DateTimeFormat=AdminDateTimeFormat;
    const NativeNumberFormat=Intl.NumberFormat;
    const AdminNumberFormat=function(locales,options){return new NativeNumberFormat('en-US',numberOptions(options))};
    AdminNumberFormat.prototype=NativeNumberFormat.prototype;AdminNumberFormat.supportedLocalesOf=NativeNumberFormat.supportedLocalesOf.bind(NativeNumberFormat);Intl.NumberFormat=AdminNumberFormat;
    ...
    const nativeNumber=Number.prototype.toLocaleString;Number.prototype.toLocaleString=function(locales,options){return nativeNumber.call(this,'en-US',numberOptions(options))};
  ...
  function ensureCurrentCss(){return new Promise((resolve,reject)=>{const existing=document.querySelector('link[data-admin-navigation-current="31"],link[data-admin-navigation-css="31"]');if(existing){if(existing.sheet)return resolve();existing.addEventListener('load',resolve,{once:true});existing.addEventListener('error',()=>reject(new Error('admin navigation css failed')),{once:true});return}const link=document.createElement('link');link.rel='stylesheet';link.href='admin-navigation.css?v=31';link.dataset.adminNavigationCurrent='31';link.addEventListener('load',resolve,{once:true});link.addEventListener('error',()=>reject(new Error('admin navigation css failed')),{once:true});document.head.appendChild(link)})}
  async function boot(){await waitForRole();const cssReady=ensureCurrentCss();const navReady=import('./admin-navigation.js?v=31');await Promise.all([cssReady,navReady]);const aside=document.querySelector('.admin-nav');if(aside&&aside.dataset.adminBuilt!=='31')aside.dataset.adminBuilt='force-current-31';document.body.dataset.adminShellReady='31';root.classList.remove('admin-pending','admin-shell-pending')}
